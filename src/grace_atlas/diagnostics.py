...
def render_diagnostics_pages(report: GapReport, graph: AtlasGraph) -> dict[str, str]:
    ...
    def page(title: str, codes: list[str], intro: str) -> str:
        ...
        lines = [
            ...
            f"Всего: **{len(items)}**",
            "",
            "Легенда: `declared` = из GRACE; `inferred` = эвристика (не auto-confirmed); "
            "`unresolved` = цель отсутствует или только stub.",
            "",
        ]
        if not items:
            lines.append("_В этой категории findings нет._")
            lines.append("")
            return "\n".join(lines)
    ...
    summary_lines = [
        ...
        "# Сводка диагностики",
        "",
        "Диагностика трассируемости (только чтение). Inferred-связи **не** подтверждены.",
        "",
        f"- Findings: **{report.summary.get('findings', 0)}**",
        f"- По severity: `{report.summary.get('by_severity', {})}`",
        f"- По коду: `{report.summary.get('by_code', {})}`",
        "",
        "## Счётчики",
        "",
        f"- Модули: {report.summary.get('modules')}",
        f"- Верификации: {report.summary.get('verifications')}",
        f"- Use cases: {report.summary.get('use_cases')}",
        f"- Файлы: {report.summary.get('files')}",
        f"- Битые ссылки: {report.summary.get('broken_references')}",
        f"- Сироты требований/UC: {report.summary.get('orphan_requirements')}",
        f"- Непроверенные модули: {report.summary.get('unverified_modules')}",
        f"- Непривязанные исходники: {report.summary.get('unmapped_files')}",
        f"- Отсутствующие файлы: {report.summary.get('missing_files')}",
        f"- Неоднозначные связи: {report.summary.get('ambiguous_links')}",
        "",
        "## Отчёты",
        "",
        ...
        "- [[Diagnostics/Unmapped-Files|Непривязанные файлы]]",
        "- [[Diagnostics/Ambiguous-Links|Неоднозначные / inferred]]",
        "",
    ]
    ...
    return {
        ...
        "Diagnostics/Broken-References.md": page(
            ...
            "Declared-пути или концы рёбер, которые нельзя сопоставить с реальными артефактами/узлами.",
        ),
        "Diagnostics/Orphan-Requirements.md": page(
            "Сироты требований / Use Cases",
            ["ORPHAN_REQUIREMENT"],
            "Use cases / requirement-like узлы без связи с flow или verification.",
        ),
        "Diagnostics/Unverified-Modules.md": page(
            ...
            "Модули без рёбер verification; записи verification без тестов.",
        ),
        "Diagnostics/Unmapped-Files.md": page(
            "Непривязанные исходные файлы",
            ["UNMAPPED_FILE"],
            "Файлы под `src/` без GRACE-ребра модуля (только scan / orphan code).",
        ),
        "Diagnostics/Ambiguous-Links.md": page(
            "Неоднозначные / inferred связи",
            ["AMBIGUOUS_LINK", "INFERRED_EDGE", "STUB_VERIFICATION"],
            "Свободный текст CrossLink и inferred-рёбра — не auto-confirmed.",
        ),
    }
